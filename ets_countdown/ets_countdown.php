...
$this->refs = 'https://prestahero.com/';
    ...
    public function hookDisplayProductCustom($params)
    {
        $specific_prices = $params['product']['specific_prices'];
        if(Configuration::get('ETS_CD_ENABLE_COUNT_DOWN') && $specific_prices && $specific_prices['to'] ) {
            if ($params['product']['specific_prices']) {

//                Tools::dieObject($specific_prices['from']);
                $this->smarty->assign(
                    array(
                        'to_specific_prices' => $specific_prices['to'],
                        'ETS_CD_TIME_UNIT_COLOR' => Configuration::get('ETS_CD_TIME_UNIT_COLOR'),
