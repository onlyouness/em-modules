...
class RegimeBanner extends Module
{

    public function __construct()
    {
        $this->name    = 'regimebanner';
        ...
        $this->bootstrap              = true;
        $this->displayName            = $this->l('Regime Banner', 'RegimeBanner');
        $this->description            = $this->l('Allows you to create foor banners');
    }
    ...
    {
        $queries   = [];
        $queries[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'regime_banner` (
            `id` INT AUTO_INCREMENT PRIMARY KEY,
            `id_feature` INT,
            `image` varchar(255),
            `active` INT default 1
        ) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=UTF8;';
        $queries[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'regime_banner_lang` (
            `id_banner` INT,
            `id_lang` INT,
            `title` varchar(250) ,
    ...
    {
        $queries   = [];
        $queries[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'regime_banner`';
        $queries[] = 'DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'regime_banner_lang`';
        foreach ($queries as $query) {
            if (! Db::getInstance()->execute($query)) {
                return false;
            }
    ...
    protected function installTab(): bool
    {
        ...
        $parentTabClassName = 'AdminBannerRegime'; // Unique class name for parent tab
        if (! Tab::getIdFromClassName($parentTabClassName)) {
            ...
            foreach ($languages as $language) {
                $parentTab->name[$language['id_lang']] = 'Regime Banner'; // Display name for parent tab
            }

            $parentTab->icon           = 'settings'; // Optional icon for the parent tab
            $parentTab->wording        = 'Regime Banner';
            $parentTab->wording_domain = 'Modules.AdminBannerRegime.Admin';
        ...
        $exists = Tab::getIdFromClassName('AdminBannerRegimeController');
        if (! $exists) {
            $tab             = new Tab();
            $tab->active     = true;
            $tab->enabled    = true;
            $tab->module     = $this->name;
            $tab->class_name = 'AdminBannerRegimeController';
            $tab->id_parent  = $parentTab->id;
            foreach ($languages as $language) {
                $tab->name[$language['id_lang']] = 'Regime Group';
            }
            $tab->icon           = '';
            $tab->wording        = 'Group banner';
            $tab->wording_domain = 'Modules.AdminBannerRegime.Admin';
            try {
                $tab->save();
            } catch (Exception $e) {
    ...
    public function hookDisplayHeader()
    {
        ...
        $this->context->controller->addCSS($this->_path . 'views/css/regimebanner.css', 'all');
        $this->context->controller->addJs($this->_path . 'views/js/regimebanner.js', 'all');
    }
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        if (file_exists(_PS_MODULE_DIR_ . 'regimebanner/js/admin.js')) {
            $controller->addJS($jsPath);
        }
    }
    public function getContent()
    {
        $homePage = Link::getUrlSmarty(['entity' => 'sf', 'route' => 'mm_regime_banner_index']);
