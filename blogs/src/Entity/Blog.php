...

{
    ...
    /**
     * @ORM\OneToMany(targetEntity="Hp\Blogs\Entity\BlogParagraph", mappedBy="blog", cascade={"persist", "remove"})
     */
    private $paragraphs;
    ...
    public function addParagraphs(BlogParagraph $paragraph): self
    {
        if (!$this->paragraphs->contains($paragraph)) {
            $this->paragraphs->add($paragraph);
            $paragraph->setBlog($this);
        }
        return $this;
    }

    public function removeParagraphs(BlogParagraph $paragraph): self
    {
        if ($this->paragraphs->contains($paragraph)) {
            $this->paragraphs->removeElement($paragraph);
            if ($paragraph->getBlog() === $this) {
                $paragraph->setBlog(null);
            }
        }
        return $this;
    }
